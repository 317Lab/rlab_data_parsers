...
binary_file = "/home/sean-wallace/Documents/rlab_data_parsers/20250925T182711Z_data_7_230400.bin"

swp_time, payload_id, volts, imu_time, acc, mag, gyr = pb.parse_all(binary_file)
print(swp_time[1,:])
plt.plot(np.arange(swp_time[1,:].size), swp_time[1,:],label='buf')
plt.plot(np.arange(swp_time[0,:].size), swp_time[0,:],label='unbuf')
plt.show()
