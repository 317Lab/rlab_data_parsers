...
sentinels = ['0x230x230x53','0x230x230x49','0x230x230x54','0x230x230x4A'] # ['##S','##I','##T','##J']
sentinel_size = 3
plotting = True
first_capture = True
...
def on_close(event):
    global plotting
    print('Close event captured at',datetime.now().astimezone().strftime("%Y/%m/%d, %H:%M:%S LT"),flush=True)
    plotting = False
...
while plotting:
    ...
    if b'TIMEOUT\n' in bytes: # shield_feed.py sends repeated 'TIMEOUT' when serial port has timed out
        ...
        break
    print("sweep sentinel: ",sentinels[0]," imu sentinel: ",sentinels[1],flush=True)
    ids_swp = list(bytes.findall(sentinels[0], bytealigned=False))
    ids_imu = list(bytes.findall(sentinels[1], bytealigned=False))
    print("ids_swp: ",ids_swp," ids_imu: ",ids_imu,flush=True)
    if buffered:
    ...
    if (num_dat_swp<=0) | (num_dat_imu<=0):
        try:
            print('DATA DROPOUT AT',datetime.now().astimezone().strftime("%Y/%m/%d, %H:%M:%S LT"),flush=True)
            print("num_dat_swp: ",num_dat_swp," num_dat_imu: ",num_dat_imu,flush=True)
