...
# 10 minute initial timeout to allow for early code execution
# Loop terminates after a 5 minute timeout with no data after initial full frame capture
# To stop recording press Enter or close figure.
...
def main():
    ...
    th.Thread(target=read_write_thread, args=(), daemon=True).start() # thread reading and writing bytes
    while plotting:
        # rawBytes = ser.read(numBytesTarget)
        # f.write(rawBytes)
        ...
        if numBytes == 0: # end recording if no bytes after timeout
            ...
            if posSWP==0 or posIMU==0: # no full messages found indicating scrambled bytes
                ...
                time.sleep(1) # only print in 1 second intervals
            else:
    ...
    plt.show()
    time.sleep(1)
    # os.rename(fn,fn[:-4]+'_'+str(payloadID[-1])+'.bin') # add payload ID to filename
