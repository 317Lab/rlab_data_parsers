...
#def repair_tcont(tdct, invalid_first=False):
##    tlims = tdct.copy() 
#    tlims = dict()    
##    tlims = []
##    tdct['test1']['imuList']
#    tlims_lst = [[tdct['test'+str(inx)]['imuList'].min(), tdct['test'+str(inx)]['imuList'].max()] for inx in np.arange(1, len(tdct.keys())+1)]
#    real_tlims = [tlims_lst[0]]
#    for inx in np.arange(1, len(tdct.keys())):
##        tlims[tkey]['Test ID']=tdct[tkey]['Test ID']
#        tkey ='test'+str(inx+1) 
#        if not tlims_lst[inx][0]>max_time: 
#            sloc = np.where(tdct[tkey]['imuList']>max_time)[0]
#            slocRpt = np.where(tdct[tkey]['imuListRpt']>max_time)[0]
##            tdct[tkey]['imuList'].sloc()
#        if tlims_lst[inx][1] > =
#
#        max_time = real_tlims[inx][1]
#
#
#
#        
#
#    for inx in np.arange(1, len(tdct.keys())+1):
#        tkey ='test'+str(inx) 
#        tlims[tkey]['tlims']=tlims_lst[inx-1]
#
#
#    return tlims, tlims_lst
GPS_time = None
pip_plttype='V'
#pip_plttype='nA'
time_lims = dict()
time_lims[1]={'timeMin': None, 'timeMax': None, 'timesToSkip': [0, 1]}
time_lims[2]={'timeMin': None, 'timeMax': None, 'timesToSkip': [0, 1]}
time_lims[3]={'timeMin': None, 'timeMax': 1944.450884, 'timesToSkip': [0, 1]}
########################################################################
...
main_figpath = "./FiguresPy-Delamere_Vacuum_Tests/"
...
if __name__ == "__main__": 
    ...
    for file_name in file_lst: 
        ...
        ############################ Padding Data #######################################
            ...
            # In[77]:
            time_dct['test'+str(test_chrono)]={'Test ID': dataFile.split('/')[-1].split('-')[1], \
                    'imuList': np.array(imuList), 'imuTime': np.array(imuTime), \
                    'imuListRpt': np.array(imuListRpt), 'imuTimeRpt': np.array(imuTimeRpt)}
    ...
    #    plottype = 'Repeat'
        if make_plots: 
            for plottype in ['Original', 'Repeat']: 
                ...
                import os
    #            fig_path = figpath+dataFile.split('/')[-1].split('-')[1]+'/'
